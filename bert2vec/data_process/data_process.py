...
def getCosineMatrix(mat):
    ...
    for i in range(N):
        for j in range(i):
            cos_mat[i][j] = cosine_similarity(np.reshape(pd.Series.to_numpy(mat.iloc[i]), (1, -1)),np.reshape(pd.Series.to_numpy(mat.iloc[j]), (1, -1)))
...
def saveReport(rowsName, rows, vectors, k = 1.5, b = 0):
    ...
    for i in vectors.keys():
        ...
        doc.add_paragraph(f"entry {i}, count: {rows[i].count}, sumBOW: {sum(rows[i].BOW.values())}",
                          style='List Bullet')
        ...
        for (j, cosSim, bowSim) in vectors[i]:
            doc.add_paragraph(f"entry {j}, count: {rows[j].count}, sumBOW: {sum(rows[j].BOW.values())}", style='List Bullet')
