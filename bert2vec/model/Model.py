...
# Get a bag-of-words of the ith word in the list
def get_bag_of_words(text: list[str], i: int) -> list[str]:
...
class Model:
    ...
    @staticmethod
    def is_table_exists(path: str):
        if not path.endswith(".dat"):
            path = f"{path}.dat"
        return os.path.isfile(path)

    # return vec by the most count
    def get_most_frequent_vec(self, word: str):
    ...
    def get_rows_by_bert(self, token: str, sentence: str):
        ...
        query_id = tokenizer.convert_tokens_to_ids(token)

        try:
            token_index = token_ids.index(query_id)
        except ValueError:
    ...
    def get_rows_by_bow_tfidf(self, word, bow: set):
    ...
    # receives a word, returns a list of most appropriate rows according to BM25 formula
    def get_rows_by_bow_bm25(self, word, bow: set, k1=1.5):
            ...
            lst += [
                (
                    ...
                    sum(
                        bow_idf_dict.get(w)  # IDF
                        * (
                            (r.BOW.get(w, 0) * (k1 + 1)) / (r.BOW.get(w, 0) + k1)
    ...
    def get_vec_by_bow(self, word: str, bow: set):
        row = self.get_row_by_bow(word, bow)
        return row.vec

    def get_row_by_bow(self, word: str, bow: set):
        lst = self.get_rows_by_bow_bm25(word, bow)
        return lst[0][0]
    ...
    def get_rows_by_count(self, word, n=5):
    ...
    def read_data(path=f"{PATH}\\{TABLE_DIR}\\{TABLE}", inMem=False):
        table = Model()
        table.inMem = inMem
        if not Model.is_table_exists(path):
