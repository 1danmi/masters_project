...
class TokenEntry(BaseModel):
    ...
    def weighted_vector_sum(vectors_count: list[tuple[np.ndarray, int]]) -> np.ndarray:
        ...
        arr = np.array([
            np.multiply(t[0], t[1] / total_count) # t[0] is the vector, t[1] is the count
            for t in vectors_count]
    ...
    def update_bow(self, bow: list[str]):
        for token in bow:
            self.bow[token] = self.bow.get(token, 0) + 1

    def unite_entries(self, token: 'TokenEntry'):
        self.update_vec_count(token.vec, token.count)
