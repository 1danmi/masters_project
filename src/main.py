...
def create_entries_db(corpus: list[str]):
    ...
    with Bert2VecModel(source_path=config().bert2vec_path, in_mem=False) as bert2vec_model:
        with Bert2VecModel(source_path=config().dest_path, in_mem=False, new_model=True) as dest_model:
            ...
            parallel_run(db_path=Path("data/temp/data.db"), corpus=corpus, func=func)
...
def main():
    ...
    book_corpus = [t["text"] for t in load_dataset("bookcorpus/bookcorpus", trust_remote_code=True)["train"]]
